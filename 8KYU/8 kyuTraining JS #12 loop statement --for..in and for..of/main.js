...
// PREP
// We are working through and object and need to loop through it
// Objects do not use for() loops, but for(in) loops as given in the example
// We need to loop through the object and push the results to an empty array
// There are a few conditionals that need to be addressed using the length method
// The object appears to have only string in it
// We then return the array

// We start by making an empty array to push to
// The for(let key in obj) where key is a variable that we link to the functions parameter (obj)
// The loop needs to check the condition if the key (variable) length is 5 and they value in key (obj[key]) is length 5
// Use the .length method to check this
// These need to be conditionals
// After the condition is checked we need to use the .push method to push the results to the empty array
